  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        />
        <Button onClick={() => setItems(Array.from({ length: count }, () => crypto.randomUUID()))}>
          Generate
        </Button>
        <Button
          variant="outline"
          onClick={() => copyToClipboard(items.join("\n"), `${items.length} UUIDs copied`)}
        >
          Copy all
        </Button>
      </div>
          ...
          {items.map((u, i) => (
            <div key={i} className="flex items-center gap-3 px-3 py-1.5 hover:bg-accent/30 group">
              ...
              <button
                ...
                className="text-xs text-muted-foreground opacity-0 group-hover:opacity-100 hover:text-foreground transition"
              >
                copy
              </button>
            </div>
...
export function QrCodeTool() {
  ...
  useEffect(() => {
    ...
    QRCode.toCanvas(
      canvasRef.current,
      text || " ",
      { width: size, margin: 2, color: { dark: "#ffffff", light: "#0a0a0a00" } },
      () => {},
    );
