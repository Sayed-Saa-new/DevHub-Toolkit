...
const WORDS =
  "the quick brown fox jumps over a lazy dog lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt ut labore et dolore magna aliqua".split(
    " ",
  );
const JOBS = [
  "Software Engineer",
...
function intBetween(r: () => number, min: number, max: number) {
...
function genValue(f: Field, r: () => number, i: number): unknown {
  switch (f.type) {
    ...
    case "email":
      return `${pick(r, FIRST).toLowerCase()}.${pick(r, LAST).toLowerCase()}${intBetween(r, 1, 99)}@${pick(r, DOMAINS)}`;
    case "username":
      return `${pick(r, FIRST).toLowerCase()}_${intBetween(r, 10, 9999)}`;
    case "avatar":
      return `https://i.pravatar.cc/128?u=${intBetween(r, 1, 10000)}`;
    case "phone":
      return `+${intBetween(r, 1, 99)} ${intBetween(r, 100, 999)}-${intBetween(r, 100, 999)}-${intBetween(r, 1000, 9999)}`;
    case "company":
      return pick(r, COMPANIES);
    case "jobTitle":
      return pick(r, JOBS);
    case "city":
      return pick(r, CITIES);
    case "country":
      return pick(r, COUNTRIES);
    case "streetAddress":
      return `${intBetween(r, 1, 9999)} ${pick(r, LAST)} St`;
    case "zipCode":
      return String(intBetween(r, 10000, 99999));
    case "int":
      return intBetween(r, f.min ?? 0, f.max ?? 100);
    case "float": {
      const min = f.min ?? 0,
        max = f.max ?? 1;
    ...
    case "isoDate":
      return new Date(Date.now() - intBetween(r, 0, 365 * 5) * 86400000).toISOString();
    case "timestamp":
      return Math.floor((Date.now() - intBetween(r, 0, 365 * 5) * 86400000) / 1000);
    case "url":
      return `https://${pick(r, DOMAINS)}/${pick(r, WORDS)}/${intBetween(r, 1, 999)}`;
    case "domain":
      return pick(r, DOMAINS);
    case "ipv4":
      return `${intBetween(r, 1, 255)}.${intBetween(r, 0, 255)}.${intBetween(r, 0, 255)}.${intBetween(r, 0, 255)}`;
    case "ipv6":
      return Array.from({ length: 8 }, () => intBetween(r, 0, 65535).toString(16)).join(":");
    case "userAgent":
      return `Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/${intBetween(r, 90, 140)}.0.0.0 Safari/537.36`;
    case "color":
      return pick(r, COLORS);
    case "hexColor":
      return "#" + intBetween(r, 0, 0xffffff).toString(16).padStart(6, "0");
    case "word":
      return pick(r, WORDS);
    case "sentence": {
...
// ---------- Serializers ----------
function toJSON(rows: Record<string, unknown>[]) {
...
function toTS(rows: Record<string, unknown>[]) {
  return `export const data = ${JSON.stringify(rows, null, 2)} as const;\n`;
}
...
export function MockData() {
  ...
  const output = useMemo(() => {
    switch (format) {
      ...
      case "sql":
        return toSQL(rows, fields, tableName || "mock_data");
      case "ts":
        return toTS(rows);
      case "xml":
        return toXML(rows);
  ...
  function addField() {
    setFields((prev) => [...prev, { id: uid(), name: `field_${prev.length + 1}`, type: "word" }]);
  }
  function reseed() {
  ...
  return (
    <div className="space-y-4">
      <Panel
        title="Schema"
        actions={
          <Button size="sm" variant="ghost" onClick={addField} className="h-7 gap-1.5 text-xs">
        ...
        <div className="divide-y divide-border">
          ...
          {fields.length === 0 && (
            <div className="p-6 text-center text-sm text-muted-foreground">
              Add a field to start generating data.
            </div>
          )}
        </div>
      </Panel>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <Field label="Seed" hint="deterministic">
          <div className="flex gap-1.5">
            ...
            <Button variant="outline" size="icon" onClick={reseed} aria-label="Randomize seed">
      ...
      <Panel
        ...
        actions={
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="sm" onClick={reseed} className="h-7 gap-1.5 text-xs">
      ...
      >
          ...
          <Tabs value={format} onValueChange={(v) => setFormat(v as Format)}>
            <TabsList className="h-8">
              {(Object.keys(FORMAT_META) as Format[]).map((k) => (
                <TabsTrigger key={k} value={k} className="h-6 px-2.5 text-xs">
                  {FORMAT_META[k].label}
                </TabsTrigger>
              ))}
            </TabsList>
        ...
        <Textarea
          readOnly
          value={output}
          className="min-h-[320px] rounded-none border-0 border-t border-border bg-background font-mono text-xs mt-3"
        />
      </Panel>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-12 gap-2 p-2.5 items-center">
          ...
          <SelectContent>
            {TYPE_GROUPS.map((g) => (
              <div key={g.label}>
                ...
                {g.items.map((it) => (
                  <SelectItem key={it.value} value={it.value} className="text-sm">
                    {it.label}
                  </SelectItem>
                ))}
              </div>
      ...
      <Button
        ...
        size="icon"
        onClick={onRemove}
        className="col-span-2 md:col-span-1 h-8 w-8 justify-self-end text-muted-foreground hover:text-destructive"
