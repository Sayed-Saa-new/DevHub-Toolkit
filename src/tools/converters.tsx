  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      <Panel
        ...
        actions={
          <>
            <Button variant="ghost" size="sm" onClick={() => setInput("")} className="h-7 text-xs">
              Clear
            </Button>
          </>
        }
        ...
        actions={
          <>
            <select
              ...
              className="h-7 text-xs bg-transparent border border-border rounded px-2"
            >
      ...
      >
        {error ? (
          <div className="p-4 font-mono text-sm text-destructive whitespace-pre-wrap min-h-[420px]">
            {error}
          </div>
        ) : (
          <pre className="p-4 font-mono text-sm min-h-[420px] whitespace-pre-wrap break-all overflow-auto max-h-[600px]">
            {output || <span className="text-muted-foreground">Formatted JSON appears here.</span>}
          </pre>
        )}
      </Panel>
    </div>
    ...
    <Tabs value={mode} onValueChange={(v) => setMode(v as "encode" | "decode")}>
      ...
      <TabsContent value="encode" className="space-y-3 mt-4">
        ...
        <Panel title="Base64 output" actions={<CopyButton text={b64} />}>
          <pre className="p-4 font-mono text-sm whitespace-pre-wrap break-all min-h-[120px]">
            {b64 || <span className="text-muted-foreground">Base64 result appears here.</span>}
          </pre>
        </Panel>
      </TabsContent>
      <TabsContent value="decode" className="space-y-3 mt-4">
        ...
        <Panel title="Plain text output" actions={<CopyButton text={text} />}>
          <pre className="p-4 font-mono text-sm whitespace-pre-wrap break-all min-h-[120px]">
            {text || <span className="text-muted-foreground">Decoded text appears here.</span>}
          </pre>
        </Panel>
      </TabsContent>
    </Tabs>
    ...
    <div className="space-y-4">
      ...
      {parsed && !("error" in parsed) && (
        <div className="grid md:grid-cols-3 gap-4">
          <Panel
            title="Header"
            actions={<CopyButton text={JSON.stringify(parsed.header, null, 2)} />}
          >
          ...
          <Panel
            title="Payload"
            actions={<CopyButton text={JSON.stringify(parsed.payload, null, 2)} />}
          >
            <pre className="p-3 font-mono text-xs whitespace-pre-wrap break-all">
          ...
          <Panel title="Signature">
            <pre className="p-3 font-mono text-xs break-all text-muted-foreground">
